...
import '../../../services/models/colive_topup_record_model.dart';
...
class ColiveCashRecordsWidget extends StatefulWidget {
  const ColiveCashRecordsWidget({super.key});

  @override
  State<ColiveCashRecordsWidget> createState() =>
      _ColiveCashRecordsWidgetState();
}

class _ColiveCashRecordsWidgetState extends State<ColiveCashRecordsWidget>
    with AutomaticKeepAliveClientMixin {
  final _apiClient = Get.find<ColiveApiClient>();

  List<ColiveTopupRecordModel> _recordList = [];
  ...
  void _onRefresh() async {
    try {
      ...
      final result = await _apiClient
          .fetchTopupRecordList('$_currentPage', '$_pageSize')
          .response;
  ...
  Widget _buildRecordItem(ColiveTopupRecordModel record) {
    return SizedBox(
      height: 76.pt,
      child: Row(
        children: [
          ...
          Column(
            ...
            children: [
              ...
              SizedBox(height: 4.pt),
              Text(
                ColiveFormatUtil.millisecondsToTime(record.createTime * 1000),
                style: ColiveStyles.body12w400.copyWith(
                  color: ColiveColors.grayTextColor,
                ),
              ),
