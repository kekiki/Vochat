...
import '../../../services/models/colive_sys_record_model.dart';
...
class ColiveSysRecordsWidget extends StatefulWidget {
  const ColiveSysRecordsWidget({super.key});

  @override
  State<ColiveSysRecordsWidget> createState() =>
      _ColiveSysRecordsWidgetState();
}

class _ColiveSysRecordsWidgetState extends State<ColiveSysRecordsWidget>
    with AutomaticKeepAliveClientMixin {
  final _apiClient = Get.find<ColiveApiClient>();

  List<ColiveSysRecordModel> _recordList = [];
  ...
  void _onRefresh() async {
    try {
      ...
      final result = await _apiClient
          .fetchSysRecordList('$_currentPage', '$_pageSize')
          .response;
  ...
  Widget _buildRecordItem(ColiveSysRecordModel record) {
    return SizedBox(
      height: 76.pt,
      child: Row(
        children: [
