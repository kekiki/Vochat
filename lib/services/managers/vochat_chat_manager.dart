...
import 'package:vochat/services/managers/vochat_chat_extension.dart';
...
import '../models/vochat_gift_base_model.dart';
import '../repositories/vochat_anchor_repository.dart';
import '../../common/event_bus/vochat_event_bus_event.dart';

class VochatChatManager {
  VochatChatManager._internal();

  static VochatChatManager? _instance;
  static VochatChatManager get instance =>
      _instance ??= VochatChatManager._internal();

  final _appDatabase = Get.find<VochatDatabase>();
  final _anchorRepository = Get.find<VochatAnchorRepository>();
  ...
  static String customerServiceName = 'vochat_customer_service';
  static const customerServiceOrderBy = 999999;
  static bool isCustomerServiceId(String id) {
    if (VochatPreference.customerServiceId.isNotEmpty) {
      return id == VochatPreference.customerServiceId;
    }
  ...
  Future<void> init() async {
    ...
    _fetchCustomerServiceInfo().then((value) {
      VochatPreference.customerServiceId = value;
    });
    // 添加事件监听器
    ...
    do {
      ...
      if (GetPlatform.isAndroid) {
        options = NIMAndroidSDKOptions(
          appKey: VochatAppMacros.nimKey,
          // sdkRootDir: VochatPathUtil.appFilePath,
          // serverConfig: config,
        );
      } else {
      ...
      if (result.isSuccess) {
        VochatLogUtil.i(_tag, "init SDK success");
        break;
      }
      if (!hasLog) {
        hasLog = true;
  ...
  Future<void> _login() async {
    ...
    do {
      ...
      final token = VochatPreference.token;
      ...
      if (result.isSuccess) {
        VochatLogUtil.i(_tag, "login success");
        break;
      }
      if (!hasLog) {
        hasLog = true;
        VochatLogUtil.e(_tag, "login failed: code=${result.code}", true);
      }
      await Future.delayed(const Duration(seconds: 3));
    } while (true);
  }
  ...
  Future<void> _logout() async {
    ...
    if (result.isSuccess) {
      VochatLogUtil.i(_tag, "logout SDK success");
    } else {
      VochatLogUtil.e(_tag, "logout SDK failed", true);
    }
  }
  ...
  Future<void> _setupEventListener() async {
    ...
    subsriptions
      ...
      for (var message in messages) {
        ...
        VochatLogUtil.i(_tag, 'Receive message: ${message.toMap().toString()}');
        VochatEventBus.instance.fire(VochatChatMessageEvent(localMessage));
      }
    }));
    ...
    subsriptions
      ...
      final localMessage = message.toLocalModel;
      if (localMessage == null) {
        VochatLogUtil.w(_tag, 'Invalid message status: ${message.toString()}');
        return;
      }
      VochatEventBus.instance.fire(VochatChatMessageEvent(localMessage));
    }));
  ...
  Future<bool> fetchConversationList() async {
    //如果没有客服就添加一个
    if (VochatPreference.customerServiceId.isEmpty) {
      VochatPreference.customerServiceId = await _fetchCustomerServiceInfo();
    }

    if (VochatPreference.customerServiceId.isNotEmpty) {
      final customerServiceConversation = await _appDatabase.chatConversationDao
          .getConversationsWithId(VochatPreference.customerServiceId);
      if (customerServiceConversation == null ||
          customerServiceConversation.isEmpty) {
        final conversation = VochatChatConversationModel.createCustomerService(
            VochatPreference.customerServiceId, customerServiceOrderBy);
  ...
  Future<String> _fetchCustomerServiceInfo() async {
    final result =
        await Get.find<VochatApiClient>().fetchCustomerServiceInfo().response;
  ...
  Future<void> _saveLocalConversationList(
    ...
    final List<VochatChatConversationModel> conversationList = [];
  ...
  Future<void> saveLocalConversation(
      VochatChatConversationModel conversation) async {
    final conversationDao = _appDatabase.chatConversationDao;
  ...
  /// 拉取消息
  Future<List<VochatChatMessageModel>> fetchMessageList({
    required String sessionId,
    VochatChatMessageModel? messageModel,
    int limit = 20,
  }) async {
    ...
    final List<VochatChatMessageModel> messageList = [];
  ...
  Future<bool> sendTextMessage({
    ...
    if (!isCustomerServiceId(sessionId)) {
      if (_hasShieldedWord(text)) {
        VochatLogUtil.e(_tag, "sendTextMessage failed: isShielded");
        ...
        if (resMessage.isSuccess && resMessage.data != null) {
          ...
          if (localMessage == null) {
            VochatLogUtil.e(_tag, 'Invalid message: ${resMessage.toString()}');
            return false;
          }
          VochatEventBus.instance.fire(VochatChatMessageEvent(localMessage));
        }
    ...
    if (!result.isSuccess) {
      VochatLogUtil.e(_tag, result.errorDetails ?? result.toString());
    }
    return result.isSuccess;
  }
  ...
  Future<bool> sendEmojiMessage({
    ...
    var attachment = NIMCustomMessageAttachment(data: {
      'type': VochatChatMessageType.emoji,
      'data': {
  ...
  Future<bool> sendGiftMessage({
    ...
    var attachment = NIMCustomMessageAttachment(data: {
      'type': VochatChatMessageType.gift,
  ...
  Future<bool> resendMessage(VochatChatMessageModel messageModel) async {
    ...
    if (result.isSuccess && result.data != null && result.data!.isNotEmpty) {
      ...
      if (!messageRes.isSuccess) {
        VochatLogUtil.e(_tag, messageRes.errorDetails ?? result.toString());
      }
